...
- `/configuracoes/integracoes` — conexão self-service de canais externos, começando pelo iFood. Autenticação e vínculo Merchant ↔ Store permanecem separados de módulos e do fluxo operacional; Pedidos, Cardápio e Entrega ficam desligados até ativação explícita de cada capability.
