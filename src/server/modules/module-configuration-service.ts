...
export type ModuleConfigurationPreview = {
  context: AccessContext;
  snapshot: StoreModuleSnapshot;
  plan: ModuleLifecyclePlan;
};

async function operationalBlockers(
...
export class ModuleConfigurationService {
  static async preview(input: {
  ...
  }): Promise<ModuleConfigurationPreview> {
    const rawContext = input.existingContext ?? (await getAccessContext());
    ...
    const blockers = input.enabled
      ? []
      : await operationalBlockers(context.organizationId, context.storeId, input.moduleKey);
    ...
    const plan = planModuleChange({
      ...
      modulesBlockedByPlan: blockedByPlan,
