...
export class CheckoutService {
  ...
  private static async requireCart(storeSlug: string, token: string) {
    ...
    if (!cart || cart.items.length === 0 || !store) throw new CheckoutError("cart_empty", "Seu carrinho está vazio");
  ...
  private static async upsertSession(admin: AdminClient, store: StoreRef, cartId: string, patch: Record<string, unknown>) {
    ...
    const { data: updated, error: updateError } = await admin.from("checkout_sessions")
      .update(values).eq("organization_id", store.organization_id).eq("store_id", store.id).eq("cart_id", cartId)
      .select("id").maybeSingle();
    if (updateError) throw updateError;
    if (updated) return updated.id;

    const { data: inserted, error: insertError } = await admin.from("checkout_sessions").insert({
      organization_id: store.organization_id, store_id: store.id, cart_id: cartId, ...values,
    }).select("id").single();
  ...
  private static async listDeliveryNeighborhoods(admin: AdminClient, store: StoreRef) {
    const { data, error } = await admin.from("delivery_neighborhoods")
      ...
      .eq("organization_id", store.organization_id).eq("store_id", store.id).eq("active", true).is("deleted_at", null)
      .order("city").order("neighborhood_name");
    if (error) throw error;
    return (data ?? []).map((item) => ({
      id: item.id, neighborhoodName: item.neighborhood_name, city: item.city, state: item.state,
  ...
  static async load(storeSlug: string, token: string, recognitionToken: string | null = null) {
    ...
    const [session, methods, menu, recognizedCustomer, deliveryNeighborhoods, growthEnabled] = await Promise.all([
      ...
      CustomerRecognitionService.resolve(cartResult.store.organization_id, cartResult.store.id, recognitionToken),
  ...
  static async saveIdentity(storeSlug: string, token: string, input: CheckoutIdentityInput) {
    const values = this.parseInput(checkoutIdentitySchema.safeParse(input), "invalid_identity", "Confira seu nome, WhatsApp e e-mail");
    const cartResult = await this.requireCart(storeSlug, token);
    const admin = createAdminClient();
    ...
    const { data: existing, error: customerError } = await admin.from("customers").select("id")
      .eq("organization_id", cartResult.store.organization_id).eq("phone_normalized", phoneNormalized).is("deleted_at", null).maybeSingle();
    if (customerError) throw customerError;
    await this.upsertSession(admin, cartResult.store, cartResult.cart.id, {
      customer_id: existing?.id ?? null, customer_name: values.name, customer_phone: values.phone,
      customer_phone_normalized: phoneNormalized, customer_email: values.email ?? null,
    });
    const { error: cartUpdateError } = await admin.from("carts")
      .update({ customer_id: existing?.id ?? null, updated_at: new Date().toISOString() })
      .eq("id", cartResult.cart.id).eq("organization_id", cartResult.store.organization_id).eq("store_id", cartResult.store.id);
  ...
  static async saveFulfillment(storeSlug: string, token: string, fulfillment: FulfillmentType) {
    const type = this.parseInput(fulfillmentTypeSchema.safeParse(fulfillment), "invalid_fulfillment", "Escolha entrega ou retirada");
    const [cartResult, menu] = await Promise.all([this.requireCart(storeSlug, token), PublicMenuService.getMenu(storeSlug)]);
    if (!menu) throw new CheckoutError("menu_unavailable", "Cardápio indisponível");
    if (type === "pickup" && !menu.settings.allow_pickup) throw new CheckoutError("pickup_disabled", "Retirada não está disponível");
    if (type === "delivery" && (!menu.settings.allow_delivery || !menu.delivery.enabled)) throw new CheckoutError("delivery_disabled", "Entrega não está disponível");
    const admin = createAdminClient();
    const { error } = await admin.rpc("checkout_set_fulfillment_internal", {
      p_store_id: cartResult.store.id, p_token_hash: hashCartToken(token), p_fulfillment_type: type, p_address: null,
      p_delivery_quote_status: type === "delivery" ? "required" : "not_required", p_delivery_fee_cents: 0,
      p_estimated_min_minutes: null, p_estimated_max_minutes: null,
    });
    if (error) throw error;
  }

  static async saveSchedule(storeSlug: string, token: string, input: CheckoutScheduleInput) {
    const values = this.parseInput(checkoutScheduleSchema.safeParse(input), "invalid_schedule", "Escolha um horário válido");
    const cartResult = await this.requireCart(storeSlug, token);
    ...
    await this.upsertSession(createAdminClient(), cartResult.store, cartResult.cart.id, { scheduled_for: scheduledFor });
  }

  private static quoteDelivery(admin: AdminClient, store: StoreRef, subtotalCents: number, address: CheckoutAddressInput) {
    return DeliveryQuoteService.quote({ admin, organizationId: store.organization_id, storeId: store.id, subtotalCents,
  ...
  static async saveAddress(storeSlug: string, token: string, input: CheckoutAddressInput) {
    const address = this.parseInput(checkoutAddressSchema.safeParse(input), "invalid_address", "Confira rua, número, bairro, cidade e UF");
    const cartResult = await this.requireCart(storeSlug, token);
    const admin = createAdminClient();
    const session = await this.getSession(admin, cartResult.cart.id);
    if (session?.fulfillment_type !== "delivery") throw new CheckoutError("delivery_not_selected", "Selecione entrega antes de informar o endereço");
    let resolvedAddress = address;
    if (address.neighborhoodId) {
      const { data: neighborhood, error: neighborhoodError } = await admin.from("delivery_neighborhoods")
        .select("id, neighborhood_name, city, state").eq("id", address.neighborhoodId)
        .eq("organization_id", cartResult.store.organization_id).eq("store_id", cartResult.store.id)
      ...
      resolvedAddress = { ...address, district: neighborhood.neighborhood_name, city: neighborhood.city, state: neighborhood.state.toUpperCase() };
    }
    ...
    const { error } = await admin.rpc("checkout_set_fulfillment_internal", {
      p_store_id: cartResult.store.id, p_token_hash: hashCartToken(token), p_fulfillment_type: "delivery",
      p_address: { postal_code: resolvedAddress.postalCode, street: resolvedAddress.street, number: resolvedAddress.number,
        complement: resolvedAddress.complement ?? "", district: resolvedAddress.district, city: resolvedAddress.city,
        state: resolvedAddress.state, reference: resolvedAddress.reference ?? "" },
      p_delivery_quote_status: quoteStatus, p_delivery_fee_cents: quote.serviceable ? quote.feeCents : 0,
    ...
    if (!quote.serviceable) {
      if (quote.reason === "minimum_order") throw new CheckoutError("delivery_minimum", `Pedido mínimo para este bairro: R$ ${(quote.minimumOrderCents / 100).toFixed(2).replace(".", ",")}`);
  ...
  static async useRecognizedAddress(storeSlug: string, token: string, recognitionToken: string | null, addressIndex: number) {
    if (!Number.isInteger(addressIndex) || addressIndex < 0) throw new CheckoutError("saved_address_invalid", "Endereço salvo inválido");
    const cartResult = await this.requireCart(storeSlug, token);
    const admin = createAdminClient();
    const session = await this.getSession(admin, cartResult.cart.id);
    if (session?.fulfillment_type !== "delivery") throw new CheckoutError("delivery_not_selected", "Selecione entrega antes de escolher o endereço");
    if (!session.customer_id) throw new CheckoutError("identity_required", "Confirme seu nome e WhatsApp antes de reutilizar um endereço");
    const recognized = await CustomerRecognitionService.resolve(cartResult.store.organization_id, cartResult.store.id, recognitionToken);
    if (!recognized || recognized.customerId !== session.customer_id) throw new CheckoutError("recognition_required", "Por segurança, informe o endereço novamente neste dispositivo");
    ...
    return this.saveAddress(storeSlug, token, {
      postalCode: saved.postalCode, street: saved.street, number: saved.number, complement: saved.complement,
      district: saved.district, city: saved.city, state: saved.state, reference: saved.reference,
    });
  }

  static async savePayment(storeSlug: string, token: string, input: CheckoutPaymentInput) {
    const values = this.parseInput(checkoutPaymentSchema.safeParse(input), "invalid_payment", "Escolha uma forma de pagamento válida");
    const cartResult = await this.requireCart(storeSlug, token);
    const admin = createAdminClient();
    ...
    if (values.method === "pix" && !session?.customer_email) throw new CheckoutError("pix_email_required", "Informe seu e-mail em Seus dados para gerar o Pix online");
    const selected = values.method === "custom"
      ? methods.find((item) => item.method === "custom" && item.customPaymentMethodId === values.customPaymentMethodId && item.enabled)
      : methods.find((item) => item.method === values.method && item.enabled);
    if (!selected) throw new CheckoutError("payment_unavailable", "Forma de pagamento indisponível");
    const changeFor = values.method === "cash" ? (values.cashChangeForCents ?? null) : null;
    if (changeFor !== null && changeFor < Number(cartResult.cart.total_cents)) throw new CheckoutError("invalid_change", "O valor para troco deve ser igual ou maior que o total");
    await this.upsertSession(admin, cartResult.store, cartResult.cart.id, {
      payment_method: values.method,
      custom_payment_method_id: values.method === "custom" ? selected.customPaymentMethodId : null,
      custom_payment_method_name: values.method === "custom" ? selected.label : null,
  ...
  static async review(storeSlug: string, token: string, recognitionToken: string | null = null) {
    ...
    if (session?.fulfillment_type === "delivery" && session.address_street && session.address_number && session.address_district && session.address_city && session.address_state) {
      const address = checkoutAddressSchema.parse({
        postalCode: session.address_postal_code ?? null, street: session.address_street, number: session.address_number,
        complement: session.address_complement, district: session.address_district, city: session.address_city,
        state: session.address_state, reference: session.address_reference,
      });
      ...
      if (session.delivery_quote_status !== quoteStatus || Number(session.delivery_fee_cents) !== fee) {
        const { error } = await admin.rpc("checkout_set_fulfillment_internal", {
          p_store_id: loaded.store.id, p_token_hash: hashCartToken(token), p_fulfillment_type: "delivery",
          p_address: { postal_code: address.postalCode, street: address.street, number: address.number,
            complement: address.complement ?? "", district: address.district, city: address.city,
            state: address.state, reference: address.reference ?? "" },
          p_delivery_quote_status: quoteStatus, p_delivery_fee_cents: fee,
          p_estimated_min_minutes: quote.serviceable ? quote.estimatedMinMinutes : null,
          p_estimated_max_minutes: quote.serviceable ? quote.estimatedMaxMinutes : null,
        });
        if (error) throw error;
    ...
    const selectedCustomStillEnabled = session?.payment_method !== "custom" || loaded.paymentMethods.some((item) =>
      item.method === "custom" && item.enabled && item.customPaymentMethodId === session?.custom_payment_method_id
    );
    const enabledMethods = loaded.paymentMethods
      ...
      .filter((item) => item.method !== "custom" || selectedCustomStillEnabled)
      .map((item) => item.method);
    if (session?.payment_method === "custom" && !selectedCustomStillEnabled) {
      while (enabledMethods.indexOf("custom") >= 0) enabledMethods.splice(enabledMethods.indexOf("custom"), 1);
    }
    const result = reviewCheckout({
      cartItemStatuses: loaded.cart.items.map((item) => item.validation_status),
      subtotalCents: Number(loaded.cart.subtotal_cents), totalCents: Number(loaded.cart.total_cents),
      minimumOrderCents: loaded.menu.settings.minimum_order_cents, canOrder: loaded.menu.operational.canOrder,
      identityComplete: Boolean(session?.customer_name && session?.customer_phone_normalized),
      fulfillmentType: session?.fulfillment_type ?? null, deliveryQuoteStatus: session?.delivery_quote_status ?? "not_required",
      paymentMethod: session?.payment_method ?? null, enabledPaymentMethods: enabledMethods,
