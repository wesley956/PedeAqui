...
export type ComplementCategorySetting = {
  id: string;
  name: string;
  active: boolean;
  selected: boolean;
...
export type PublicComplementProduct = {
  id: string;
  name: string;
...
export type PublicComplementCategory = {
  id: string;
  name: string;
  products: PublicComplementProduct[];
};
...
export class ComplementCategoryService {
  static async loadAdminSettings(): Promise<{ businessType: string; categories: ComplementCategorySetting[] }> {
    ...
    return {
      ...
      categories: (categories ?? []).map((category) => ({
        ...
        selected: selectedMap.has(category.id) || category.id === suggestedId,
        sortOrder: selectedMap.get(category.id) ?? Number(category.sort_order ?? 0),
        suggestedDefault: category.id === suggestedId,
      })),
    };
  }
  ...
  static async replaceSettings(rows: Array<{ categoryId: string; sortOrder: number }>) {
    ...
    if (!context.storeId) throw new Error("An active store is required");
    const admin = createAdminClient();
    const normalizedRows = rows.map((row) => ({ category_id: row.categoryId, sort_order: row.sortOrder }));
  ...
  static async loadPublic(storeSlug: string, excludeProductId?: string | null, previewLimit = 4): Promise<PublicComplementCategory[]> {
    ...
    const { data: store, error: storeError } = await admin.from("stores")
      .select("id,organization_id,status,business_type")
      .ilike("slug", storeSlug).in("status", ["active", "temporarily_closed"]).maybeSingle();
    if (storeError) throw storeError;
    if (!store) return [];

    const { data: settings, error: settingsError } = await admin.from("store_complement_categories")
      .select("category_id,sort_order").eq("organization_id", store.organization_id).eq("store_id", store.id).order("sort_order");
    if (settingsError) throw settingsError;
    const categoryIds = (settings ?? []).map((row) => row.category_id);
    if (categoryIds.length === 0) return [];

    const { data: categories, error: categoriesError } = await admin.from("categories")
      .select("id,name,active,sort_order").eq("organization_id", store.organization_id).eq("store_id", store.id).in("id", categoryIds).is("deleted_at", null);
    if (categoriesError) throw categoriesError;
    ...
    const { data: products, error: productsError } = await admin.from("products")
      .select("id,category_id,name,description,image_url,price_cents,promotional_price_cents,active,availability,sort_order")
      .eq("organization_id", store.organization_id).eq("store_id", store.id).in("category_id", categoryIds)
      .eq("active", true).eq("availability", "available").is("deleted_at", null).order("sort_order").order("name");
    ...
    if (productIds.length > 0) {
      const { data: links, error: linksError } = await admin.from("product_modifier_groups")
        .select("product_id").eq("organization_id", store.organization_id).eq("store_id", store.id).in("product_id", productIds);
    ...
    return categoryIds.flatMap((categoryId) => {
      ...
      if (!category) return [];
      const categoryProducts = eligibleProducts.filter((product) => product.category_id === categoryId).slice(0, previewLimit).map((product) => ({
        id: product.id,
        name: product.name,
