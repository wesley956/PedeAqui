...
}).refine((value) => value.allowPickup || value.allowDelivery, {
  message: "At least one fulfillment mode must remain enabled",
});
...
export const publicModifierSchema = z.object({
  id: z.string().uuid(),
  name: z.string(),
...
export const publicProductSummarySchema = z.object({
  ...
  preparation_time_minutes: z.number().int().nonnegative(),
...
export const publicHourSchema = z.object({
  weekday: z.number().int().min(0).max(6),
  opens_at: clockSchema,
  closes_at: clockSchema,
