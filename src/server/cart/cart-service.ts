...
export class CartService {
  private static async resolveStore(admin: AdminClient, slug: string): Promise<StoreRef> {
    const { data: store, error } = await admin.from("stores").select("id, organization_id, slug, name, status, business_type, timezone").ilike("slug", slug).in("status", ["active", "temporarily_closed"]).maybeSingle();
  ...
  private static async assertAcceptingOrders(admin: AdminClient, store: StoreRef) {
    if (store.status !== "active" || !store.accepting_orders) throw new PricingError("store_unavailable", "A loja não está aceitando pedidos agora");
    const { data: hours, error } = await admin.from("store_hours").select("weekday, opens_at, closes_at, closes_next_day").eq("organization_id", store.organization_id).eq("store_id", store.id).eq("active", true);
    if (error) throw error;
    const schedule = (hours ?? []).map((period) => ({ weekday: period.weekday, opens_at: String(period.opens_at).slice(0, 5), closes_at: String(period.closes_at).slice(0, 5), closes_next_day: period.closes_next_day }));
    if (!isOpenAt(schedule, store.timezone)) throw new PricingError("store_unavailable", "A loja está fora do horário de atendimento");
  }
  ...
  private static async loadGasSaleContext(admin: AdminClient, store: StoreRef, productId: string): Promise<GasSaleContext | null> {
    ...
    const { data: profile, error } = await admin.from("product_gas_profiles").select("exchange_enabled, container_sale_enabled, require_container_choice, container_surcharge_cents, active").eq("organization_id", store.organization_id).eq("store_id", store.id).eq("product_id", productId).maybeSingle();
    if (error) throw error;
    if (!profile?.active) return null;
    return { exchange_enabled: Boolean(profile.exchange_enabled), container_sale_enabled: Boolean(profile.container_sale_enabled), require_container_choice: Boolean(profile.require_container_choice), container_surcharge_cents: Number(profile.container_surcharge_cents ?? 0) };
  }
  ...
  static async addItem(input: AddCartItemInput, existingToken?: string | null) {
    ...
    const [, product] = await Promise.all([this.assertAcceptingOrders(admin, store), this.loadPricingProduct(admin, store, values.productId)]);
    ...
    const priced = PricingService.priceItem(product, selectedModifiers, values.quantity);
    ...
    const commonArgs = { p_organization_id: store.organization_id, p_store_id: store.id, p_token_hash: tokenHash, p_expires_at: expiresAt, p_product_id: product.id, p_product_name: product.name, p_product_image_url: product.imageUrl, p_unit_base_price_cents: priced.baseUnitPriceCents, p_quantity: values.quantity, p_note: values.note ?? "", p_modifiers: priced.modifiers };
    const { data, error } = values.gasSaleMode ? await admin.rpc("cart_add_gas_item_internal", { ...commonArgs, p_sale_mode: values.gasSaleMode }) : await admin.rpc("cart_add_item_internal", commonArgs);
    if (error) throw error;
    return { token, store, data };
  }

  private static async fetchCart(admin: AdminClient, store: StoreRef, tokenHash: string) {
    const { data: cart, error } = await admin.from("carts").select("id, customer_id, subtotal_cents, discount_cents, delivery_fee_cents, total_cents, expires_at, updated_at, coupon_id, coupon_code_snapshot, coupon_discount_cents, cashback_redeem_requested_cents, cashback_discount_cents, loyalty_redeem_requested_points, loyalty_discount_cents").eq("organization_id", store.organization_id).eq("store_id", store.id).eq("token_hash", tokenHash).eq("status", "active").gt("expires_at", new Date().toISOString()).maybeSingle();
    if (error) throw error;
    if (!cart) return null;
    const { data: items, error: itemsError } = await admin.from("cart_items").select("id, product_id, product_name_snapshot, product_image_url_snapshot, quantity, note, unit_base_price_cents, unit_modifiers_price_cents, unit_segment_price_cents, unit_total_price_cents, line_total_cents, validation_status, price_changed_at, created_at").eq("cart_id", cart.id).order("created_at");
    if (itemsError) throw itemsError;
    const itemIds = (items ?? []).map((item) => item.id);
    const [modifiersResult, gasResult] = itemIds.length > 0 ? await Promise.all([
      admin.from("cart_item_modifiers").select("cart_item_id, modifier_group_id, modifier_id, group_name_snapshot, modifier_name_snapshot, unit_price_cents, quantity").in("cart_item_id", itemIds).order("created_at"),
  ...
  static async getCart(storeSlug: string, token?: string | null) {
    ...
    for (const item of cart.items) {
      ...
      try {
        const selectedModifiers = item.modifiers.map((modifier) => ({ modifierId: modifier.modifier_id, quantity: Number(modifier.quantity ?? 1) }));
        const priced = PricingService.priceItem(product, selectedModifiers, item.quantity);
        ...
        const priceChanged = item.unit_base_price_cents !== priced.baseUnitPriceCents || item.unit_modifiers_price_cents !== priced.modifiersUnitPriceCents || Number(item.unit_segment_price_cents ?? 0) !== segmentUnitPrice || item.unit_total_price_cents !== unitTotalPriceCents || Number(item.line_total_cents) !== lineTotalCents;
        const snapshotChanged = item.product_name_snapshot !== product.name || item.product_image_url_snapshot !== product.imageUrl;
        updates.push({ item_id: item.id, validation_status: "valid", product_name: product.name, product_image_url: product.imageUrl ?? "", unit_base_price_cents: priced.baseUnitPriceCents, unit_modifiers_price_cents: priced.modifiersUnitPriceCents, unit_segment_price_cents: segmentUnitPrice, unit_total_price_cents: unitTotalPriceCents, line_total_cents: lineTotalCents, price_changed: priceChanged, modifiers: priced.modifiers });
