...
export class FiscalReadService {
  static async load(){
    ...
    const [storeResult,integrationsResult,profileResult,documentsResult,jobsResult,ordersResult,productsResult,profilesResult]=await Promise.all([
      ...
      admin.from("fiscal_documents").select("id,order_id,integration_id,model,environment,status,series,document_number,access_key,protocol,cancellation_protocol,rejection_code,rejection_message,queued_at,processing_at,authorized_at,rejected_at,cancelled_at,contingency_at,created_at,updated_at").eq("organization_id",context.organizationId).eq("store_id",storeId).order("created_at",{ ascending:false }).limit(120),
    ...
    return {
      ...
      documents:documentsResult.data??[],jobs:jobsResult.data??[],eligibleOrders,
