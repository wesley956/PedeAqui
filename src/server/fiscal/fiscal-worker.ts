...
import { FiscalArtifactService } from "@/server/fiscal/fiscal-artifact-service";
...
async function transition(admin:ReturnType<typeof createAdminClient>,job:ClaimedJob,status:string,input?:{ code?:string|null;message?:string|null;accessKey?:string|null;protocol?:string|null;cancellationProtocol?:string|null;metadata?:Record<string,unknown> }){ const { error }=await admin.rpc("fiscal_transition_internal",{ p_fiscal_document_id:job.fiscal_document_id,p_to_status:status,p_idempotency_key:transitionKey(job,status),p_event_type:`fiscal.${status}`,p_provider_code:input?.code??null,p_message:input?.message??null,p_access_key:input?.accessKey??null,p_protocol:input?.protocol??null,p_cancellation_protocol:input?.cancellationProtocol??null,p_actor_user_id:null,p_metadata:input?.metadata??{} }); if(error) throw error; }
...
async function loadEnvelope(admin:ReturnType<typeof createAdminClient>,job:ClaimedJob){
  ...
  const document=documentResult.data as DocumentRow; const integration=integrationResult.data as IntegrationRow; if(!integration.active) throw new Error("Fiscal integration is inactive");
...
async function processJob(admin:ReturnType<typeof createAdminClient>,job:ClaimedJob,workerId:string,resolver:FiscalProviderResolver){
  try{
    const { document,integration,envelope,context }=await loadEnvelope(admin,job); const provider=resolver(integration.provider_key); if(!provider||provider.key!==integration.provider_key) throw new Error(`Fiscal provider not registered: ${integration.provider_key}`);
    if(job.job_type==="issue"){
      ...
      if(result.status==="contingency"){ if(currentStatus==="queued"||currentStatus==="processing") await transition(admin,job,"contingency",{ code:result.code,message:result.message,metadata:{ provider_document_id:result.providerDocumentId??null } }); }
        ...
        if(result.status==="authorized"&&currentStatus==="processing"){
          await transition(admin,job,"authorized",{ code:result.code,message:result.message,accessKey:result.accessKey,protocol:result.protocol,metadata:{ provider_document_id:result.providerDocumentId??null } });
          if(result.artifacts?.xml) await FiscalArtifactService.store({ organizationId:job.organization_id,storeId:job.store_id,fiscalDocumentId:job.fiscal_document_id,artifacts:result.artifacts });
    ...
    if(job.job_type==="cancel"){
      ...
      const reason=typeof job.payload?.reason==="string"?job.payload.reason:"Cancelamento solicitado"; const result=await provider.cancel({ ...envelope,reason },context);
...
export async function runFiscalWorker(resolver:FiscalProviderResolver,input?:{ workerId?:string;limit?:number }){ const admin=createAdminClient(); const workerId=input?.workerId??`fiscal-worker-${process.pid}`; const limit=Math.max(1,Math.min(input?.limit??10,50)); const { data,error }=await admin.rpc("fiscal_claim_jobs_internal",{ p_worker_id:workerId,p_limit:limit,p_lease_seconds:120 }); if(error) throw error; const jobs=(data??[]) as ClaimedJob[]; for(const job of jobs) await processJob(admin,job,workerId,resolver); return { claimed:jobs.length }; }
