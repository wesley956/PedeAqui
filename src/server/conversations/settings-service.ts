...
const settingsSchema = z.object({
  ...
  if (!validateGreetingTemplate(value.greetingTemplate)) ctx.addIssue({ code: "custom", path: ["greetingTemplate"], message: "A saudação deve usar {link}, pode usar {restaurante} e não pode conter URL manual." });
  if (!validateGreetingFallback(value.greetingFallbackMessage)) ctx.addIssue({ code: "custom", path: ["greetingFallbackMessage"], message: "Fallback inválido ou com URL manual." });
});
...
export class ConversationSettingsService {
  ...
  static async health(): Promise<WhatsAppChannelHealth> {
    ...
    if (error) throw error; if (!settings?.whatsapp_enabled) return emptyHealth("disabled", "WhatsApp desabilitado nesta unidade.");
    if (settings.provider !== "meta_cloud" || !settings.whatsapp_phone_number_id || !settings.access_token_secret_ref || !settings.app_secret_secret_ref) return emptyHealth("misconfigured", "Configuração incompleta do canal WhatsApp.");
    ...
    catch (error) { return emptyHealth("misconfigured", error instanceof Error ? error.message : "Configuração do WhatsApp inválida."); }
    try { const inspected = await new WhatsAppCloudProvider(accessToken).inspectPhoneNumber(settings.whatsapp_phone_number_id); return { status: "connected", message: "Credencial e Phone Number ID aceitos pela WhatsApp Cloud API.", displayPhoneNumber: inspected.displayPhoneNumber, verifiedName: inspected.verifiedName, qualityRating: inspected.qualityRating, graphVersion }; }
    catch (error) { if (error instanceof WhatsAppProviderError && !error.retryable) return emptyHealth("invalid_credentials", "A Meta rejeitou a credencial ou o Phone Number ID configurado.", graphVersion); return emptyHealth("provider_unavailable", "Não foi possível consultar a Meta agora. Tente novamente sem alterar as credenciais.", graphVersion); }
  }
  static async save(input: ConversationSettingsInput) {
    ...
    if (values.whatsappEnabled && (!values.phoneNumberId || !values.accessTokenSecretRef || !values.appSecretSecretRef)) throw new Error("Para habilitar o WhatsApp, informe phone number id e referências dos dois secrets.");
    if (values.aiEnabled && !values.botEnabled) throw new Error("A IA depende do bot habilitado.");
    if (values.greetingEnabled && (!values.whatsappEnabled || !values.botEnabled)) throw new Error("A saudação automática depende do WhatsApp e do bot habilitados.");
