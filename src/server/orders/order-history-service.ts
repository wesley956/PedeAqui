...
function resolveDateRange(period: OrderHistoryPeriod, selectedDate: string | undefined, timeZone: string, now = new Date()) {
  ...
  if (period === "date") {
    startDate = selectedDate && parseCalendarDate(selectedDate) ? selectedDate : today;
...
export class OrderHistoryService {
  static async list(input: {
    ...
    const selectedDate = input.date && parseCalendarDate(input.date)
      ? input.date
      : period === "date" ? today : "";
    ...
    let countQuery = admin.from("orders")
