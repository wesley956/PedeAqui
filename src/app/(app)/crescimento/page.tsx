...
function segmentSummary(value: unknown) {
  ...
  if (Number(rules.orders_count_max) > 0) labels.push(`até ${Number(rules.orders_count_max)} pedidos`);
    ...
    <main className={styles.root}>
      <header className={styles.hero}>
        <div>
          ...
          <p>Organize fidelidade, cupons, grupos de clientes e campanhas em um só lugar, com foco no que ajuda o restaurante a gerar novas compras.</p>
        </div>
      </header>
      ...
      <section id="clientes" className={styles.section}>
        <div className={styles.sectionHeader}><div><h2>Grupos de clientes</h2><p>O total do grupo é separado de quem pode receber uma campanha no WhatsApp.</p></div></div>
        <div className={styles.summaryGrid}>{data.groupSummaries.filter((group) => group.group_kind === "preset").map((group) => <article className={styles.summaryCard} key={group.group_key}><span className={styles.status} data-active={Number(group.members) > 0}>Grupo pronto</span><strong>{group.name}</strong><small>{Number(group.members)} no grupo · {Number(group.eligible_whatsapp)} elegíveis no WhatsApp</small></article>)}</div>
        ...
        <div className={styles.chips}>{data.segments.map((segment) => { const count = data.groupSummaries.find((group) => group.segment_id === segment.id); return <span key={segment.id} className={styles.chip}><strong>{segment.name}</strong>{segmentSummary(segment.rules)} · {Number(count?.members ?? 0)} no grupo / {Number(count?.eligible_whatsapp ?? 0)} elegíveis</span>; })}</div>
        <details className={styles.details}><summary>Criar grupo personalizado</summary><form action={createSegmentAction} className={styles.detailsBody}><div className={styles.formGrid}><label className={styles.label}>Nome<input className={styles.field} name="name" required placeholder="Clientes VIP" /></label><label className={styles.label}>Pedidos mínimos<input className={styles.field} name="ordersCountMin" type="number" min={1} /></label><label className={styles.label}>Pedidos máximos<input className={styles.field} name="ordersCountMax" type="number" min={1} /></label><label className={styles.label}>Gasto mínimo<input className={styles.field} name="totalSpentMin" inputMode="decimal" /></label><label className={styles.label}>Ticket médio mínimo<input className={styles.field} name="averageTicketMin" inputMode="decimal" /></label><label className={styles.label}>Inativo há pelo menos<input className={styles.field} name="inactiveDaysMin" type="number" min={1} placeholder="dias" /></label><label className={styles.label}>Comprou nos últimos<input className={styles.field} name="lastOrderDaysMax" type="number" min={1} placeholder="dias" /></label></div><div className={styles.checks}><label><input type="checkbox" name="hasCashbackBalance" /> Com cashback</label><label><input type="checkbox" name="hasLoyaltyBalance" /> Com pontos</label></div><label className={styles.label}>Descrição<input className={styles.field} name="description" maxLength={500} /></label><div><button className={styles.primary} type="submit">Criar grupo</button></div></form></details>
      </section>
