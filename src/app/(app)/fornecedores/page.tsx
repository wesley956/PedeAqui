...
export default async function SuppliersPage() {
  ...
  return <section className={styles.page}>
    <header className={styles.header}><div className={styles.headerCopy}><p className="muted">ESTOQUE E COMPRAS</p><h1>Fornecedores</h1><p className="muted">Cadastre de quem você compra e configure prazo, pedido mínimo e itens fornecidos para esta unidade.</p></div><div className={styles.links}><Link href="/compras" className={styles.link}>← Compras</Link><Link href="/estoque" className={styles.link}>Estoque</Link></div></header>
    ...
    <div className={styles.columns}>
      <div className={styles.supplierList}>{data.suppliers.length===0 ? <article className={`card ${styles.card}`}><p className="muted">Nenhum fornecedor cadastrado.</p></article> : data.suppliers.map((supplier)=>{const config=supplier.config;return <article key={supplier.id} className={`card ${styles.supplier}`}>
        <div className={styles.top}><div className={styles.identity}><strong>{supplier.name}</strong><span className={styles.meta}>{supplier.tax_document||supplier.legal_name||"Sem documento informado"}</span>{supplier.email||supplier.phone ? <span className={styles.meta}>{[supplier.email,supplier.phone].filter(Boolean).join(" · ")}</span>:null}</div><div className={styles.side}>{config?.active ? <SemanticStatus tone="success" icon="✓" label="Ativo nesta unidade"/> : <SemanticStatus tone="warning" icon="!" label="Ainda não usado aqui"/>}{config ? <span className={styles.meta}>{config.lead_time_days} dia(s) · mínimo {money(config.minimum_order_cents)}</span>:null}</div></div>
        {supplier.catalog.length ? <div className={styles.catalog}><strong>O que este fornecedor vende para você</strong>{supplier.catalog.map((row)=>{const item=data.inventory.find((candidate)=>candidate.id===row.inventory_item_id);return <div key={row.inventory_item_id} className={styles.catalogRow}><span>{item?.name??"Insumo"}{row.is_preferred?" · preferencial":""}</span><span className="muted">{row.purchase_unit_label} = {String(row.base_units_per_purchase_unit)} {item?.base_unit??""} · {money(row.last_unit_cost_cents)}</span></div>;})}</div>:<span className={styles.meta}>Nenhum insumo associado nesta unidade.</span>}
        {data.canManage ? <div className={styles.actions}><details><summary>Condições de compra</summary><div className={styles.actionBody}><SupplierConfigForm supplierId={supplier.id} active={config?.active??false} leadTimeDays={config?.lead_time_days??0} minimumOrderCents={config?.minimum_order_cents??0} notes={config?.notes??null}/></div></details>{config?.active ? <details><summary>Adicionar ou atualizar item</summary><div className={styles.actionBody}><SupplierCatalogForm supplierId={supplier.id} inventory={data.inventory}/></div></details>:null}</div>:null}
      </article>;})}</div>
      {data.canManage ? <aside><article className={`card ${styles.card}`}><div><h2>Novo fornecedor</h2><p className="muted">Cadastre o fornecedor e depois informe as condições de compra desta unidade.</p></div><SupplierCreateForm/></article></aside>:null}
    </div>
  </section>;
}
