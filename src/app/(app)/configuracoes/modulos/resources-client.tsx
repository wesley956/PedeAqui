...
import { applyModuleChangeInlineAction, requestModuleActivationAction, type ModuleInlineActionState } from "@/features/modules/actions";
...
export function ResourcesClient({ resources, focusKey = null }: { resources: readonly ResourceItem[]; focusKey?: string | null }) {
  ...
  return <>
    <div className={styles.controls}>
      <label className={styles.search}><span aria-hidden>⌕</span><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Procurar recurso..." /></label>
    ...
    <div className={styles.list}>{visible.map((resource) => <ResourceRow resource={resource} key={`${resource.key}:${resource.enabled}:${resource.requestPending}`} highlighted={resource.key === focusKey} />)}</div>
...
function ResourceRow({ resource, highlighted }: { resource: ResourceItem; highlighted: boolean }) {
  ...
  const sellableRequest = !resource.enabled && !resource.canActivate && resource.monthlyPriceCents != null;
  return <article id={`resource-${resource.key}`} className={styles.resource} data-highlighted={highlighted || undefined}>
    <div className={styles.resourceMain}>
      <div className={styles.copy}>
        <div className={styles.titleLine}><strong>{resource.label}</strong>{resource.core ? <span className={styles.locked}>Sempre ativo</span> : resource.enabled ? <span className={styles.active}>Ativo</span> : <span className={styles.inactive}>Inativo</span>}</div>
        <p>{resource.description}</p>
        {resource.dependencies.length > 0 ? <small>Usa também: {resource.dependencies.join(", ")}</small> : null}
        {sellableRequest ? <small><strong>Adicional: + {money(resource.monthlyPriceCents!)}/mês</strong></small> : null}
      </div>
      {!resource.core ? sellableRequest ? (
        resource.requestPending ? <span className={styles.unavailable}>Solicitação enviada</span> :
        <form action={requestModuleActivationAction} className={styles.actionForm}><input type="hidden" name="moduleKey" value={resource.key} /><button type="submit" className={styles.primaryButton}>Solicitar ativação</button></form>
      ) : <form action={action} className={styles.actionForm}>
        <input type="hidden" name="moduleKey" value={resource.key} /><input type="hidden" name="enabled" value={String(targetEnabled)} />
