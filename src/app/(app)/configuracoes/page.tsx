...
const definitions = {
  ...
  payments: { title: "Formas de pagamento", description: "Dinheiro, cartões, Pix e opções disponíveis ao cliente.", href: "/configuracoes/pagamentos", permissions: [PERMISSIONS.STORES_VIEW], keywords: ["pix", "cartao", "credito", "debito", "dinheiro", "pagamento"] },
  delivery: { title: "Entrega", description: "Bairros, taxas, prazo e frete grátis.", href: "/configuracoes/entrega", permissions: [PERMISSIONS.STORES_VIEW], keywords: ["bairro", "taxa", "frete", "entrega", "delivery", "prazo"], moduleKey: "deliveries" },
  ...
  printing: { title: "Impressão automática", description: "Impressoras, locais e acompanhamento da fila.", href: "/configuracoes/impressoes", permissions: [PERMISSIONS.PRINTING_VIEW], keywords: ["impressora", "imprimir", "impressao", "print agent", "cozinha"] },
...
export default async function SettingsPage() {
  ...
  const areas: SettingsHubArea[] = [
    ...
    { key: "orders", icon: "🧾", title: "Pedidos e atendimento", description: "Como os pedidos entram, avançam e são atendidos.", items: [resolve(definitions.flow), resolve(definitions.dining), resolve(definitions.cash)].filter((item): item is SettingsHubItem => Boolean(item)) },
    ...
    { key: "printing", icon: "🖨️", title: "Impressão automática", description: "Impressoras, setores e fila de impressão.", items: [resolve(definitions.printing)].filter((item): item is SettingsHubItem => Boolean(item)) },
