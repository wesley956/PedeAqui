...
export default async function PlatformOrderDiagnosticPage({params}:{params:Promise<{orderId:string}>}){
 ...
 return <div className={styles.page}>
  ...
  <section className={styles.section}><div className={styles.sectionHeader}><div><h2>Pedido → Evento → Fila → WhatsApp → Entrega</h2><p>Correlação persistida e somente leitura. Telefones e conteúdo das mensagens não são exibidos.</p></div></div>
   <div className={styles.advancedNote}>Pedido <code>{data.correlation.orderId}</code> · Carrinho <code>{data.correlation.cartId}</code> · Checkout <code>{data.correlation.checkoutSessionId}</code></div>
   <div className={styles.healthList}>{data.whatsappChain.length?data.whatsappChain.map(item=><div key={item.id} className={styles.healthRow}><span><strong>{item.notificationType} · {item.outcomeLabel}</strong><small>Evento: {item.domainEventId??"ausente"} · Job: {item.id} · Mensagem: {item.messageId??"não criada"}</small><small>Meta: {item.providerMessageId??"sem ID"} · Destinatário protegido: {item.recipientMasked??"não resolvido"}</small>{item.failureCategory?<small>{item.failureCategory} · {item.errorCode??"sem código"}{item.errorDetail?` · ${item.errorDetail}`:""}</small>:null}</span><span>Tentativas: {item.attempts}<small>Próxima tentativa: {item.nextAttemptAt?dateTime.format(new Date(item.nextAttemptAt)):"não agendada"}</small></span></div>):<div className={styles.empty}>Nenhuma notificação transacional foi gerada para este pedido.</div>}</div>
  </section>
