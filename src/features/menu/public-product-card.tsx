...
export function PublicProductCard({ product, storeSlug, showImage, canOrder }: { product: Product; storeSlug: string; showImage: boolean; canOrder: boolean }) {
  ...
  return <Link href={`/m/${storeSlug}/produto/${product.id}`} className={`${styles.product} ${showImage ? "" : styles.productNoImage} ${unavailable ? styles.productUnavailable : ""}`} aria-label={`${product.name}, ${soldOut ? "esgotado" : money(price)}`}>
    <div className={styles.productCopy}>
      ...
      <div className={styles.priceRow}><strong className={styles.price}>{money(price)}</strong>{hasPromotion ? <span className={styles.oldPrice}>{money(product.price_cents)}</span> : null}{!unavailable ? <span className={styles.viewHint}>Ver opções →</span> : <span className={styles.unavailableHint}>{soldOut ? "Indisponível para adicionar" : "Consulte agora; pedidos fechados"}</span>}</div>
    </div>
    {showImage ? product.image_url ? (
      ...
      <img src={product.image_url} alt="" width={104} height={104} loading="lazy" decoding="async" className={styles.image} />
