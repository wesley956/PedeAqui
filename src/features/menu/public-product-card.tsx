  ...
  return <Link href={`/m/${storeSlug}/produto/${product.id}`} className={`${styles.product} ${showImage ? "" : styles.productNoImage} ${unavailable ? styles.productUnavailable : ""}`} aria-label={`${product.name}, ${soldOut ? "esgotado" : money(price)}`}>
    <div className={styles.productCopy}>
      ...
      <div className={styles.priceRow}>
        {hasPromotion ? <span className={styles.oldPrice}>{money(product.price_cents)}</span> : null}
        <strong className={styles.price}>{money(price)}</strong>
        {!unavailable ? <span className={styles.viewHint}>Ver opções →</span> : <span className={styles.unavailableHint}>{soldOut ? "Indisponível para adicionar" : "Consulte agora; pedidos fechados"}</span>}
      </div>
    </div>
