...
type PurchaseServerAction = (previous: PurchaseActionState, formData: FormData) => Promise<PurchaseActionState>;
function useIdempotentAction(serverAction: PurchaseServerAction) {
  const keyRef = useRef<string | null>(null);
  return useActionState(async (previous: PurchaseActionState, formData: FormData) => {
    ...
    if (next.ok) keyRef.current = null;
    return next;
  }, initial);
}
...
export function SupplierCreateForm() {
  ...
  return <form action={action} style={{ display: "grid", gap: 8 }}>
    <input name="name" required minLength={2} maxLength={160} placeholder="Nome do fornecedor" aria-label="Nome do fornecedor" style={inputStyle} />
    <input name="legalName" maxLength={160} placeholder="Razão social (opcional)" aria-label="Razão social" style={inputStyle} />
    <input name="taxDocument" maxLength={40} placeholder="CNPJ/Documento (opcional)" aria-label="CNPJ ou documento" style={inputStyle} />
    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}><input name="email" type="email" placeholder="E-mail" aria-label="E-mail" style={inputStyle} /><input name="phone" placeholder="Telefone" aria-label="Telefone" style={inputStyle} /></div>
    ...
    <textarea name="notes" maxLength={2000} placeholder="Observações" aria-label="Observações" style={{ ...inputStyle, minHeight: 70 }} />
...
export function SupplierConfigForm({ supplierId, active, leadTimeDays, minimumOrderCents, notes }: { supplierId: string; active: boolean; leadTimeDays: number; minimumOrderCents: number | string; notes?: string | null }) {
  ...
  return <form action={action} style={{ display: "grid", gap: 7 }}><input type="hidden" name="supplierId" value={supplierId} />
    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 7 }}><input name="leadTimeDays" type="number" min={0} max={365} defaultValue={leadTimeDays} aria-label="Prazo padrão em dias" style={inputStyle} /><input name="minimumOrder" inputMode="decimal" defaultValue={moneyInput(minimumOrderCents)} aria-label="Pedido mínimo em reais" style={inputStyle} /></div>
    <input name="notes" maxLength={1000} defaultValue={notes ?? ""} placeholder="Observações da unidade" aria-label="Observações da unidade" style={inputStyle} /><label style={{ fontSize: 12 }}><input name="active" type="checkbox" defaultChecked={active} /> Ativo nesta unidade</label>
...
export function SupplierCatalogForm({ supplierId, inventory }: { supplierId: string; inventory: Array<{ id: string; name: string; base_unit: string }> }) {
  ...
  return <form action={action} style={{ display: "grid", gap: 7 }}><input type="hidden" name="supplierId" value={supplierId} />
    <select name="inventoryItemId" required defaultValue="" aria-label="Insumo" style={inputStyle}><option value="" disabled>Selecione o insumo</option>{inventory.map((item) => <option key={item.id} value={item.id}>{item.name} · {item.base_unit === "unit" ? "un" : item.base_unit}</option>)}</select>
    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 7 }}><input name="supplierSku" placeholder="Código no fornecedor" aria-label="Código no fornecedor" style={inputStyle} /><input name="purchaseUnitLabel" required placeholder="Ex.: caixa 12un / saco 5kg" aria-label="Unidade de compra" style={inputStyle} /></div>
    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 7 }}><input name="baseUnitsPerPurchaseUnit" required inputMode="decimal" placeholder="Unidades-base por embalagem" aria-label="Unidades-base por embalagem" style={inputStyle} /><input name="unitCostInput" inputMode="decimal" placeholder="Custo por embalagem (R$)" aria-label="Custo por embalagem" style={inputStyle} /></div>
...
export function PurchaseCreateForm({ suppliers, catalog }: { suppliers: Array<{ id: string; name: string }>; catalog: CatalogRow[] }) {
  const [state, action, pending] = useIdempotentAction(createPurchaseAction);
  ...
  return <form action={action} style={{ display: "grid", gap: 10 }}>
    <select name="supplierId" required value={supplierId} onChange={(e) => setSupplierId(e.target.value)} aria-label="Fornecedor" style={inputStyle}><option value="" disabled>Fornecedor</option>{suppliers.map((supplier) => <option key={supplier.id} value={supplier.id}>{supplier.name}</option>)}</select>
    {rows.length === 0 ? <div className="muted" style={{ fontSize: 12 }}>Cadastre os insumos deste fornecedor antes de criar a compra.</div> : rows.map((row) => <div key={row.inventory_item_id} style={{ display: "grid", gridTemplateColumns: "minmax(150px,1fr) 110px 120px", gap: 7, alignItems: "end" }}>
      ...
      <input name="quantity" inputMode="decimal" placeholder="Qtd." aria-label={`Quantidade de ${row.inventory?.name ?? "insumo"}`} style={inputStyle} />
      <input name="unitCostInput" inputMode="decimal" defaultValue={moneyInput(row.last_unit_cost_cents)} placeholder="R$/embalagem" aria-label={`Custo de ${row.inventory?.name ?? "insumo"}`} style={inputStyle} />
    </div>)}
...
export function ReceivePurchaseForm({ orderId, items }: { orderId: string; items: PurchaseItem[] }) {
  const [state,action,pending] = useIdempotentAction(receivePurchaseAction);
  const open = items.filter((item) => Number(item.received_purchase_quantity) < Number(item.ordered_purchase_quantity));
  return <form action={action} style={{ display: "grid", gap: 8 }}><input type="hidden" name="purchaseOrderId" value={orderId} />
    {open.map((item) => <div key={item.id} style={{ display: "grid", gridTemplateColumns: "minmax(140px,1fr) 100px 120px", gap: 7, alignItems: "end" }}><div><strong style={{ fontSize: 12 }}>{item.inventory_name_snapshot}</strong><div className="muted" style={{ fontSize: 10 }}>{item.received_purchase_quantity}/{item.ordered_purchase_quantity} {item.purchase_unit_label_snapshot}</div><input type="hidden" name="purchaseOrderItemId" value={item.id} /></div><input name="quantity" inputMode="decimal" placeholder="Recebido" aria-label={`Quantidade recebida de ${item.inventory_name_snapshot}`} style={inputStyle} /><input name="unitCostInput" inputMode="decimal" defaultValue={moneyInput(item.unit_cost_cents)} aria-label={`Custo recebido de ${item.inventory_name_snapshot}`} style={inputStyle} /></div>)}
    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 7 }}><input name="reference" maxLength={120} placeholder="NF / referência" aria-label="Nota fiscal ou referência" style={inputStyle} /><input name="notes" maxLength={2000} placeholder="Observação" aria-label="Observação do recebimento" style={inputStyle} /></div>
...
export function CorrectReceiptForm({ orderId, receiptId, items }: { orderId: string; receiptId: string; items: PurchaseItem[] }) {
  const [state,action,pending] = useIdempotentAction(correctPurchaseReceiptAction);
  return <form action={action} style={{ display: "grid", gap: 7 }}><input type="hidden" name="purchaseOrderId" value={orderId} /><input type="hidden" name="receiptId" value={receiptId} />
    {items.map((item) => <div key={item.id} style={{ display: "grid", gridTemplateColumns: "minmax(140px,1fr) 100px 120px", gap: 7, alignItems: "end" }}><div><strong style={{ fontSize: 12 }}>{item.inventory_name_snapshot}</strong><div className="muted" style={{ fontSize: 10 }}>Use + para complementar e − para estornar</div><input type="hidden" name="purchaseOrderItemId" value={item.id} /></div><input name="quantityDelta" inputMode="decimal" placeholder="Ex.: -0,5" aria-label={`Correção de quantidade de ${item.inventory_name_snapshot}`} style={inputStyle} /><input name="unitCostInput" inputMode="decimal" defaultValue={moneyInput(item.unit_cost_cents)} aria-label={`Custo corrigido de ${item.inventory_name_snapshot}`} style={inputStyle} /></div>)}
    <input name="reason" required minLength={3} maxLength={500} placeholder="Motivo da correção" aria-label="Motivo da correção" style={inputStyle} /><button disabled={pending} style={{ ...buttonStyle, background: "var(--surface-3, #333)" }}>{pending ? "Corrigindo…" : "Registrar correção"}</button><Feedback state={state} />
  </form>;
}
