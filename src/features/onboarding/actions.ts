...
import { BUSINESS_TYPES, MODULE_PRESETS, isModuleKey, modulesForPreset } from "@/modules/module-catalog";
...
export async function bootstrapOrganizationAction(formData: FormData) {
  ...
  const customModules = formData.getAll("modules")
    .map(String)
    .filter(isModuleKey);
  const enabledModules = modulesForPreset(parsed.data.businessType, parsed.data.preset, customModules);
  const supabase = await createClient();

  const { error: profileError } = await supabase.from("profiles").upsert({ id: user.id, status: "active" }, { onConflict: "id" });
  if (profileError) {
    logger.error("onboarding_profile_upsert_failed", { userId: user.id, errorCode: profileError.code, errorMessage: profileError.message });
  ...
  for (let attempt = 0; attempt < MAX_STORE_SLUG_ATTEMPTS; attempt += 1) {
    ...
    const { data, error } = await supabase.rpc("bootstrap_organization_modular", {
      ...
      p_module_preset: parsed.data.preset,
    ...
    if (isStoreSlugConflict(error) && attempt < MAX_STORE_SLUG_ATTEMPTS - 1) {
      logger.warn("onboarding_store_slug_conflict", { userId: user.id, attempt: attempt + 1 });
      continue;
    }
    logger.error("onboarding_bootstrap_failed", { userId: user.id, attempt: attempt + 1, errorCode: error.code, errorMessage: error.message });
    redirect("/onboarding?error=bootstrap_failed");
  }

  if (!bootstrapData || typeof bootstrapData !== "object") {
    logger.error("onboarding_bootstrap_missing_result", { userId: user.id, attempts: MAX_STORE_SLUG_ATTEMPTS });
    redirect("/onboarding?error=bootstrap_failed");
  }
