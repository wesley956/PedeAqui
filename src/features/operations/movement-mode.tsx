...
export function MovementMode({ storeId, initialOrders, workflowMode, manualDeliveryMode, healthIssues }: { storeId: string; initialOrders: OrderManagerRow[]; workflowMode: BoardWorkflowMode; manualDeliveryMode: boolean; healthIssues: OperationalHealthIssue[] }) {
  ...
  const tasks = useMemo(() => orders.map((order) => ({ order, action: primaryActionForOrder(order, workflowMode, manualDeliveryMode), profile: profileFor(order), severity: severityFor(order, now) }))
    ...
    .sort((a, b) => rank[a.severity] - rank[b.severity] || Date.parse(a.order.created_at) - Date.parse(b.order.created_at)), [orders, workflowMode, manualDeliveryMode, profile, query, now]);
  return <div className={styles.mode}>
    ...
    <section className={styles.queue} aria-label="Próximas ações">
      {tasks.map(({ order, action, severity, profile: taskProfile }) => action ? <article key={order.id} data-severity={severity}>
        ...
        <div className={styles.primary}><OrderActionForm orderId={order.id} intent={action.intent} label={action.label} tone={action.tone} /><Link href={`/pedidos/${order.id}`}>Ver detalhes</Link></div>
      </article> : null)}
