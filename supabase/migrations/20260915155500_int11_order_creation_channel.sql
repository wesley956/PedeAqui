-- INT-11: make the source channel part of canonical checkout order creation.
-- Existing 3-argument callers remain backward-compatible and keep digital_menu.
...
begin
  ...
  if p_channel not in ('digital_menu','whatsapp') then raise exception 'invalid checkout order channel'; end if;
  ...
  where organization_id=v_cart.organization_id and store_id=v_cart.store_id and cart_id=v_cart.id for update;
  ...
  v_growth:=private.resolve_growth_benefits(
    v_cart.organization_id,v_cart.store_id,v_customer_id,p_channel,v_cart.subtotal_cents,
  ...
  insert into public.orders(
    ...
    v_display_number,p_channel,v_checkout.fulfillment_type,'pending_confirmation',v_payment_status,'pending_confirmation','pending',
  ...
  values(v_cart.organization_id,v_cart.store_id,'order.created','order',v_order_id,
    jsonb_build_object('display_number',v_display_number,'channel',p_channel,'fulfillment_type',v_checkout.fulfillment_type,
      'subtotal_cents',v_cart.subtotal_cents,'discount_cents',v_discount,'total_cents',v_total),'pending',0,now());
...
-- Preserve every existing caller. The old signature delegates to the new canonical contract.
create or replace function public.create_order_from_checkout_internal(
  p_store_id uuid,
  p_token_hash text,
  p_order_access_token_hash text
...
set search_path to ''
as $function$
  select public.create_order_from_checkout_internal(
    p_store_id,
    p_token_hash,
