...
-- Credencial permanente é do System User do PedeAqui; token do cliente é transitório.
-- O PIN de registro por número fica protegido no Supabase Vault.
...
create or replace function public.whatsapp_channel_store_registration_pin_internal(
  p_store_id uuid,
...
begin
  ...
  select registration_pin_secret_id into v_secret_id
  ...
  if v_secret_id is null then
    select vault.create_secret(p_registration_pin, v_name, 'PedeAqui WhatsApp Cloud API registration PIN') into v_secret_id;
  else
    perform vault.update_secret(v_secret_id, p_registration_pin, v_name, 'PedeAqui WhatsApp Cloud API registration PIN');
  end if;

  update public.store_conversation_settings
  set registration_pin_secret_id = v_secret_id, updated_at = now()
  where store_id = v_store.id;
  return v_secret_id;
end;
$$;
revoke all on function public.whatsapp_channel_store_registration_pin_internal(uuid,text) from public, anon, authenticated;
grant execute on function public.whatsapp_channel_store_registration_pin_internal(uuid,text) to service_role;

create or replace function public.whatsapp_channel_registration_pin_internal(
  p_store_id uuid
) returns text
language sql
security definer
set search_path = ''
as $$
  select p.decrypted_secret
  from public.store_conversation_settings c
  join vault.decrypted_secrets p on p.id = c.registration_pin_secret_id
  where c.store_id = p_store_id
  limit 1
$$;
revoke all on function public.whatsapp_channel_registration_pin_internal(uuid) from public, anon, authenticated;
grant execute on function public.whatsapp_channel_registration_pin_internal(uuid) to service_role;
