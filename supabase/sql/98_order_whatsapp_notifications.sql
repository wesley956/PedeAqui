...
comment on table public.order_notification_contexts is
  'Contexto interno para links transacionais. tracking_access_token não pode ser exposto em logs, auditoria ou consultas públicas.';
...
create table if not exists public.order_whatsapp_notifications (
  ...
  notification_type text not null check (notification_type in ('order_received','payment_paid','pickup_ready','out_for_delivery','delivered')),
  ...
  constraint order_whatsapp_notifications_order_same_store_fk
    foreign key (organization_id, store_id, order_id)
    references public.orders (organization_id, store_id, id) on delete cascade,
  constraint order_whatsapp_notifications_once_per_type unique (organization_id, order_id, notification_type)
);
...
create or replace function public.order_notification_store_context_internal(p_order_id uuid, p_tracking_access_token text)
returns void
language plpgsql
security invoker
set search_path = ''
...
begin
  if char_length(coalesce(p_tracking_access_token,'')) not between 32 and 128 then raise exception 'invalid tracking access token'; end if;
  select * into v_order from public.orders where id = p_order_id;
  if v_order.id is null then raise exception 'order not found'; end if;

  insert into public.order_notification_contexts (order_id, organization_id, store_id, tracking_access_token, updated_at)
  values (v_order.id, v_order.organization_id, v_order.store_id, p_tracking_access_token, now())
  on conflict (order_id) do update set tracking_access_token = excluded.tracking_access_token, updated_at = now();
end;
$$;
...
begin
  ...
  where organization_id = v_store.organization_id and store_id = v_store.id and channel = 'whatsapp'
    and (phone_normalized = v_phone or external_id = v_phone)
  order by case when phone_normalized = v_phone then 0 else 1 end, created_at limit 1 for update;

  if v_contact.id is null then
    begin
      insert into public.contacts (organization_id, store_id, channel, external_id, phone_normalized, name, customer_id, updated_at)
      values (v_store.organization_id, v_store.id, 'whatsapp', v_phone, v_phone,
  ...
  where organization_id = v_store.organization_id and store_id = v_store.id and contact_id = v_contact.id
    and channel = 'whatsapp' and status <> 'closed'
  order by opened_at desc limit 1 for update;

  if v_conversation.id is null then
    begin
      insert into public.conversations (organization_id, store_id, contact_id, channel, status, opened_at, updated_at)
      values (v_store.organization_id, v_store.id, v_contact.id, 'whatsapp', 'bot', now(), now())
  ...
  return jsonb_build_object('conversation_id', v_conversation.id, 'contact_id', v_contact.id, 'external_id', v_contact.external_id);
end;
$$;
...
set search_path = ''
as $$
declare v_type text; v_fulfillment_type text;
begin
  ...
  insert into public.order_whatsapp_notifications (organization_id, store_id, order_id, domain_event_id, notification_type)
  values (new.organization_id, new.store_id, new.entity_id, new.id, v_type)
...
create trigger domain_events_enqueue_order_whatsapp_notification after insert on public.domain_events
for each row execute function private.enqueue_order_whatsapp_notification_from_event();

create or replace function public.order_notification_claim_internal(p_worker_id text, p_limit integer default 20)
...
begin
  ...
  return query
  with candidates as (
    select q.id from public.order_whatsapp_notifications q
    where ((q.status in ('pending','failed') and q.available_at <= now())
      or (q.status = 'processing' and q.locked_until < now()))
    order by q.available_at, q.created_at, q.id for update skip locked limit p_limit
  )
  update public.order_whatsapp_notifications q set
    status = 'processing', attempts = q.attempts + 1, locked_by = trim(p_worker_id),
...
set search_path = ''
as $$
declare v_row public.order_whatsapp_notifications%rowtype; v_retry integer;
begin
  ...
  where id = p_notification_id and status = 'processing' and locked_by = trim(p_worker_id)
