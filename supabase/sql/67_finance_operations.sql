...
declare
  ...
  v_account_delta bigint; v_description text; v_source_type text; v_metadata jsonb;
begin
  ...
  if v_existing.id is not null then
    if v_existing.store_id is distinct from p_store_id or v_existing.obligation_id is distinct from p_obligation_id
      ...
      or v_existing.competence_date is distinct from p_competence_date or v_existing.source_type is distinct from v_source_type
      or v_existing.source_id is distinct from p_source_id or v_existing.transfer_group_id is distinct from p_transfer_group_id
      or v_existing.description is distinct from v_description or v_existing.metadata is distinct from v_metadata then
  ...
  insert into public.financial_transactions(
    ...
    v_source_type,p_source_id,p_transfer_group_id,trim(p_idempotency_key),v_description,v_metadata,p_actor_user_id,coalesce(p_occurred_at,now())
  ) returning * into v_tx;
...
language plpgsql security invoker set search_path='' as $$
declare
  v_source public.financial_accounts%rowtype; v_target public.financial_accounts%rowtype; v_group uuid;
  v_out public.financial_transactions%rowtype; v_in public.financial_transactions%rowtype; v_out_key text; v_in_key text;
begin
  if p_actor_user_id is null then raise exception 'finance actor is required'; end if;
  ...
  if char_length(trim(coalesce(p_idempotency_key,''))) not between 8 and 240 then raise exception 'invalid financial transfer idempotency key'; end if;
  ...
  v_group:=(substr(md5(v_source.organization_id::text||':transfer:'||trim(p_idempotency_key)),1,8)||'-'||substr(md5(v_source.organization_id::text||':transfer:'||trim(p_idempotency_key)),9,4)||'-'||substr(md5(v_source.organization_id::text||':transfer:'||trim(p_idempotency_key)),13,4)||'-'||substr(md5(v_source.organization_id::text||':transfer:'||trim(p_idempotency_key)),17,4)||'-'||substr(md5(v_source.organization_id::text||':transfer:'||trim(p_idempotency_key)),21,12))::uuid;
  v_out_key:='finance:'||encode(extensions.digest((trim(p_idempotency_key)||':out')::text,'sha256'),'hex');
  v_in_key:='finance:'||encode(extensions.digest((trim(p_idempotency_key)||':in')::text,'sha256'),'hex');
  v_out:=private.finance_insert_transaction(v_source.organization_id,v_source.store_id,null,v_source.id,null,'transfer','out',1,p_amount_cents,null,
    'financial_transfer',v_group,v_group,v_out_key,trim(p_reason),'{}'::jsonb,p_actor_user_id,coalesce(p_occurred_at,now()));
  v_in:=private.finance_insert_transaction(v_target.organization_id,v_target.store_id,null,v_target.id,null,'transfer','in',1,p_amount_cents,null,
    'financial_transfer',v_group,v_group,v_in_key,trim(p_reason),'{}'::jsonb,p_actor_user_id,coalesce(p_occurred_at,now()));
  if v_out.transfer_group_id<>v_in.transfer_group_id then raise exception 'financial transfer pair mismatch'; end if;
  insert into public.audit_logs(organization_id,store_id,actor_user_id,action,entity_type,entity_id,after_data)
  select v_source.organization_id,v_source.store_id,p_actor_user_id,'finance.transfer','financial_transaction',v_out.id,
    jsonb_build_object('transfer_group_id',v_group,'target_account_id',v_target.id,'amount_cents',p_amount_cents)
  where not exists(select 1 from public.audit_logs a where a.organization_id=v_source.organization_id and a.action='finance.transfer' and a.entity_id=v_out.id);
...
language plpgsql security invoker set search_path='' as $$
declare
  v_store public.stores%rowtype; v_category public.financial_categories%rowtype; v_ob public.financial_obligations%rowtype;
  v_rec public.financial_transactions%rowtype; v_set public.financial_transactions%rowtype; v_source_id uuid;
  v_rec_key text; v_set_key text;
begin
  if p_actor_user_id is null then raise exception 'finance actor is required'; end if;
  if p_direction not in ('in','out') then raise exception 'invalid financial direction'; end if;
  if p_amount_cents is null or p_amount_cents<=0 then raise exception 'financial amount must be positive'; end if;
  if char_length(trim(coalesce(p_description,'')))<2 then raise exception 'financial description required'; end if;
  if char_length(trim(coalesce(p_idempotency_key,''))) not between 8 and 240 then raise exception 'invalid manual finance idempotency key'; end if;
  select * into v_store from public.stores where id=p_store_id and status='active'; if v_store.id is null then raise exception 'store unavailable'; end if;
  ...
  v_source_id:=(substr(md5(v_store.organization_id::text||':manual:'||trim(p_idempotency_key)),1,8)||'-'||substr(md5(v_store.organization_id::text||':manual:'||trim(p_idempotency_key)),9,4)||'-'||substr(md5(v_store.organization_id::text||':manual:'||trim(p_idempotency_key)),13,4)||'-'||substr(md5(v_store.organization_id::text||':manual:'||trim(p_idempotency_key)),17,4)||'-'||substr(md5(v_store.organization_id::text||':manual:'||trim(p_idempotency_key)),21,12))::uuid;
  v_rec_key:='finance:'||encode(extensions.digest((trim(p_idempotency_key)||':recognition')::text,'sha256'),'hex');
  v_set_key:='finance:'||encode(extensions.digest((trim(p_idempotency_key)||':settlement')::text,'sha256'),'hex');
  ...
  if v_ob.competence_date<>p_competence_date or v_ob.due_date<>coalesce(p_due_date,p_competence_date) or v_ob.description<>trim(p_description) then
    raise exception 'financial idempotency key reused with different payload';
  end if;
  v_rec:=private.finance_insert_transaction(v_store.organization_id,v_store.id,v_ob.id,null,v_category.id,'recognition',p_direction,1,p_amount_cents,p_competence_date,
    'manual',v_source_id,null,v_rec_key,trim(p_description),'{}'::jsonb,p_actor_user_id,now());
  if p_account_id is not null then
    v_set:=private.finance_insert_transaction(v_store.organization_id,v_store.id,v_ob.id,p_account_id,null,'settlement',p_direction,1,p_amount_cents,null,
      'manual',v_source_id,null,v_set_key,'Liquidação imediata','{}'::jsonb,p_actor_user_id,now());
  else
    select * into v_set from public.financial_transactions where organization_id=v_store.organization_id and idempotency_key=v_set_key;
    if v_set.id is not null then raise exception 'financial idempotency key reused with different payload'; end if;
  end if;
  select * into v_ob from public.financial_obligations where id=v_ob.id;
  insert into public.audit_logs(organization_id,store_id,actor_user_id,action,entity_type,entity_id,after_data)
  select v_store.organization_id,v_store.id,p_actor_user_id,'finance.manual_entry_created','financial_obligation',v_ob.id,
    jsonb_build_object('direction',p_direction,'amount_cents',p_amount_cents,'category_id',p_category_id,'settled',p_account_id is not null)
  where not exists(select 1 from public.audit_logs a where a.organization_id=v_store.organization_id and a.action='finance.manual_entry_created' and a.entity_id=v_ob.id);
