...
describe("final order options UI", () => {
  ...
  it("offers an optional schedule interpreted in the store timezone", () => { expect(page).toContain("saveCheckoutScheduleAction"); expect(page).toContain('type="datetime-local"'); expect(summary).toContain("Horário solicitado"); });
});
