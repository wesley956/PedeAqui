...
describe("final order options UI", () => {
  ...
  it("does not expose technical review states to the customer", () => { expect(page).not.toContain("Pendente de revisão"); expect(page).not.toContain("Ajustes necessários"); expect(page).not.toContain("Pronto para confirmar"); expect(page).not.toContain("CheckoutReviewState"); });
  it("keeps one visible confirmation while review and creation stay server authoritative", () => { expect(page).toContain("confirmCheckoutOrderAction"); expect(page).not.toContain("reviewCheckoutAction"); expect(confirmAction).toContain("createOrderFromCheckoutAction"); expect(orderService).toContain("CheckoutService.review(storeSlug, token)"); });
