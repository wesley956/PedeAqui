...
describe("Operational order diagnostics [341]",()=>{
 ...
 it("does not expose direct state mutations",()=>{expect(service).not.toMatch(/from\(\"orders\"\).*update/s);expect(service).not.toMatch(/from\(\"payments\"\).*update/s);expect(service).not.toMatch(/from\(\"deliveries\"\).*update/s);expect(page).toContain("Não existe botão para forçar");});
