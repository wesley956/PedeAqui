...
describe("modular resolver rollout", () => {
  ...
  it("targets store, organization and cohort in deterministic precedence", () => {
    expect(resolveModuleRollout({ enabled: true, storeIds: ["store-a"], organizationIds: ["org-a"], cohorts: ["pilot"] }, context).reason)
      .toBe("store_target");
    expect(resolveModuleRollout({ enabled: true, organizationIds: ["org-a"], cohorts: ["pilot"] }, context).reason)
      .toBe("organization_target");
    expect(resolveModuleRollout({ enabled: true, cohorts: ["pilot"] }, context).reason)
      .toBe("cohort_target");
  });
