...
function signature(secret: string, signedDataId: string, requestId: string, ts: string) {
...
describe("Mercado Pago webhook signature [328]", () => {
  it("accepts the documented HMAC manifest with an alphanumeric data id lowercased", () => {
    const secret = "webhook-test-secret";
    const dataId = "ORD01JTEST123";
    const requestId = "request-abc";
    const ts = "1786812345";
    expect(validateMercadoPagoWebhookSignature({
      xSignature: signature(secret, dataId.toLowerCase(), requestId, ts),
  ...
  it("rejects tampered data, request id and digest", () => {
    ...
    const requestId = "request-abc";
    const ts = "1786812345";
    const xSignature = signature(secret, dataId.toLowerCase(), requestId, ts);
