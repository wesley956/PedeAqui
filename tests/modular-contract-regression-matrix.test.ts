...
describe("modular contract regression matrix", () => {
  ...
  it("requires module + permission + entitlement + dependencies together", () => {
    ...
    expect(resolveModuleAvailability({
      definition,
    ...
    }).reason).toBe("not_in_plan");

    expect(resolveModuleAvailability({
      definition,
      businessType: "restaurant",
      storeEnabled: true,
      activeModuleKeys: new Set<ModuleKey>(["deliveries"]),
      grantedPermissions: new Set([permission]),
      entitlementAllowed: true,
    }).reason).toBe("missing_dependency");
  ...
  it("activation cascades declared dependencies and deactivation protects active dependents", () => {
    const activation = planModuleActivation({
      moduleKey: "driver",
      businessType: "restaurant",
      enabledModuleKeys: new Set<ModuleKey>(["orders"]),
    });
    ...
    const deactivation = planModuleDeactivation({
      moduleKey: "deliveries",
      enabledModuleKeys: new Set<ModuleKey>(["orders", "deliveries", "driver"]),
    });
  ...
  it("blocks module shutdown when an operational blocker is present", () => {
    const result = planModuleDeactivation({
      moduleKey: "deliveries",
      enabledModuleKeys: new Set<ModuleKey>(["orders", "deliveries"]),
      operationalBlockers: ["open_delivery"],
    });
