...
function Import-LegacyEnvironment {
  ...
  if (-not $AppUrl) {
    $urlMatch = [regex]::Match($content, '(?im)^set\s+"PEDEAQUI_URL=([^"]+)"\s*  $identity = [Security.Principal.WindowsIdentity]::GetCurrent()
...
try {
  ...
  if (-not $Token) {
    $tokenMatch = [regex]::Match($content, '(?im)^set\s+"PEDEAQUI_PRINT_AGENT_TOKEN=([^"]+)"\s*  $identity = [Security.Principal.WindowsIdentity]::GetCurrent()
  $principal = New-Object Security.Principal.WindowsPrincipal($identity)
  if (-not $principal.IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator)) {
    throw "A instalacao precisa ser executada como administrador."
  }
